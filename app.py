...
# =========================
POS_LABELS = [
    "面料舒适","质量很好","有助于锻炼","有助于缓解疼痛","保暖","舒适贴合","有压缩感",
    "抓握式有效","合身","有助于关节炎/扳机指","增加手指灵活","促进血液循环",
    "耐用","缓解不适","轻盈","覆盖整个手指","有助于防止肿胀"
]
...
SEEDS_POS = {
    ...
    "质量很好": ["well made", "good quality", "质量"],
    "有助于缓解疼痛": ["pain relief", "relieve pain", "arthritis"],
    "舒适贴合": ["fits well", "snug", "贴合"],
    "有压缩感": ["compression"],
    "抓握式有效": ["grip", "grippy"],
    "耐用": ["durable", "last long"],
}

SEEDS_NEG = {
    "没有作用/没有效果": ["no effect", "doesn't work"],
    "尺码太小": ["too small", "runs small", "tight"],
    "尺码太大": ["too big", "loose"],
...
def learn_polarity_weights(texts, ratings, min_df=3):
    ...
    for t, r in zip(texts, ratings):
        toks = tokenize(t)
        if r <= 3: neg.update(toks)
        elif r == 5: pos.update(toks)

    weights = {}
    for tok in set(neg) | set(pos):
        fn, fp = neg[tok], pos[tok]
        if fn + fp < min_df: continue
...
# 7. UI
# =========================
st.title("🏷️ 评论自动打标系统（区分好评其他 / 差评其他）")
...
if uploaded:
    ...
    # =========================
    st.subheader("📊 标签分布")
    st.plotly_chart(px.bar(df["Tag_Label"].value_counts().reset_index(),
